...
\begin{equation}
s^*=\frac{\sqrt{149}}{\sqrt{149}+\sqrt{51}}\approx0.630896,
\end{equation}
...
\end{equation}
while the floor remains slack at the candidate. If $q<q_L$, $L$ can cut price enough to exploit the high-$L$ continuation before the floor activates. If $q>q_U$ while still slack at the candidate, the off-path protection of $R$ becomes strong enough to create a profitable $R$ deviation. Thus a regulation that is nonbinding in normal operation can be too weak or too strong solely through its effect on off-equilibrium continuations.
